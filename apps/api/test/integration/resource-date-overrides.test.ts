...
import type { DateOverrideConfiguration } from "../../src/modules/availability/organization-date-overrides.js";
import { replaceResourceDateOverride } from "../../src/modules/availability/resource-date-overrides-service.js";
...
async function fixture() {
  ...
  const request = (
    ...
    app.inject({
      method,
      url: `/api/organizations/${targetId}/resources/${targetResourceId}/availability/date-overrides/${encodeURIComponent(targetDate)}`,
...
describe("Resource date overrides", () => {
  it("returns inherit without copying the weekly schedule", async () => {
    ...
    expect(response.json()).toEqual(representation(f.resourceId, inherit));
      ...
      for (const configuration of [
        ...
        for (let attempt = 0; attempt < 2; attempt++) {
          ...
          expect(response.json()).toEqual(
            representation(f.resourceId, configuration),
          );
  ...
  it("edits deactivated Resources without changing lifecycle or publication", async () => {
    ...
    expect((await f.request("GET")).json()).toEqual(
      representation(f.resourceId, custom()),
    );
    expect(
      await db
        .selectFrom("resource")
  ...
  it.each(["archived", "suspended"] as const)(
    ...
    async (state) => {
      ...
      const get = await f.request("GET");
      expect(get.statusCode).toBe(200);
      expect(get.json()).toEqual(representation(f.resourceId, custom()));
    },
  );
  ...
  it.each([
    ...
    expect(
      await replaceResourceDateOverride({
        ...f,
        date: invalidDate,
        mode: "closed",
        intervals: [],
  ...
  it.each([
    ...
    async (targetDate) => {
      ...
      expect(put.json()).toEqual(
        representation(f.resourceId, closed, targetDate),
      );
      expect(
        (
          await f.request(
  ...
  it.each(invalidConfigurations)(
    ...
    async (_name, configuration) => {
      ...
      expect(
        await replaceResourceDateOverride({ ...f, date, ...configuration }),
      ).toEqual({ ok: false, reason: "invalid_date_override" });
      expect(await f.read()).toEqual(before);
    },
  );
  ...
  it("scopes replacements to one date and Organization", async () => {
    ...
    ).toEqual(representation(f.resourceId, closed, "2026-10-06"));
  ...
  it("enforces SQL overlap per date/Organization while allowing adjacency", async () => {
    ...
    await expect(
      ...
      code: "23P01",
      constraint: "resource_date_override_no_overlap",
    });
    await db
      .insertInto("resource_date_override_interval")
      .values([
  ...
  it("enforces parent/tenant foreign keys and cascades Organization deletion", async () => {
    ...
    await expect(
      db.insertInto("resource_date_override_interval").values(row).execute(),
  ...
  it.each(["closed", "custom"] as const)(
    ...
    async (mode) => {
      ...
      const before = await f.read();
      await sql`ALTER TABLE resource_date_override_interval ADD CONSTRAINT test_insert_failure CHECK (end_minute <> 1001)`.execute(
        db,
      );
      try {
        const response = await f.request(
        ...
        expect(await f.read()).toEqual(before);
        expect((await f.request("GET")).json()).toEqual(
          representation(f.resourceId, original),
        );
      } finally {
        await sql`ALTER TABLE resource_date_override_interval DROP CONSTRAINT test_insert_failure`.execute(
          db,
        );
      }
    },
  ...
  it("serializes concurrent replacements from different actors", async () => {
    ...
    const results = await Promise.all([
      replaceResourceDateOverride({ ...f, date, ...first }),
    ...
    expect(results).toEqual([
      { ok: true, override: representation(f.resourceId, first) },
      { ok: true, override: representation(f.resourceId, second) },
    ]);
    expect([
      representation(f.resourceId, first),
      representation(f.resourceId, second),
    ]).toContainEqual((await f.request("GET")).json());
  });

  it("reverses and reapplies migration 0014 without modifying its Organization", async () => {
    const f = await fixture();
    await f.request("PUT", custom());
    await db.transaction().execute(async (trx) => {
      ...
      const { rows } = await sql<{
        parent: string | null;
        child: string | null;
      }>`SELECT to_regclass('resource_date_override')::text AS parent, to_regclass('resource_date_override_interval')::text AS child`.execute(
        trx,
      );
  ...
  it.each(["GET", "PUT"] as const)(
    "hides foreign and missing Resources for %s",
    async (method) => {
      const f = await fixture();
      const other = await fixture();
      await f.request("PUT", custom());
  ...
  it("rejects cross-tenant Resource parents through the composite foreign key", async () => {
    const f = await fixture();
    const other = await fixture();
    await expect(
      db
      ...
      code: "23503",
      constraint: "resource_date_override_resource_organization_fk",
    });
  });

  it("isolates different Resources on the same date within one Organization", async () => {
    const f = await fixture();
    ...
    await db
      .insertInto("resource_date_override_interval")
      .values({ ...parent, start_minute: 600, end_minute: 840 })
      .execute();
    expect((await f.request("PUT", inherit)).statusCode).toBe(200);
    expect(
      (
      ...
      representation(second.id, custom([{ startMinute: 600, endMinute: 840 }])),
    );
  });

  it("stores a custom extra-day configuration alongside recurring closure and Organization date closure", async () => {
    const f = await fixture();
    await db
    ...
    await db
      ...
      .values({ organization_id: f.organizationId, local_date: date })
      .execute();
    expect((await f.request("GET")).json()).toEqual(
      representation(f.resourceId, inherit),
    );
    expect((await f.request("PUT", custom())).statusCode).toBe(200);
    expect((await f.request("GET")).json()).toEqual(
      representation(f.resourceId, custom()),
    ...
    expect(
      await db
        ...
        .where("organization_id", "=", f.organizationId)
        .execute(),
    ).toEqual([{ organization_id: f.organizationId, local_date: date }]);
    expect(
      await db
  ...
  it("cascades parent and intervals when the Resource is deleted", async () => {
    const f = await fixture();
    await f.request("PUT", custom());
